...
# ═══════════════════════════════════════════════════════════════════════════
# CARGA DE ARCHIVOS — siempre usa st.file_uploader (compatible con GCS)
# ═══════════════════════════════════════════════════════════════════════════
...
# Banner informativo si GCS está habilitado
if _GCS_AVAILABLE:
    st.markdown("""
    <div style="background: linear-gradient(135deg, #e8f8f5, #d1f2eb); border-radius: 10px; 
        ...
        <span style="font-size: 1.1rem; font-weight: 600; color: #0D5F5D;">
            ☁️ Cloud Storage disponible
        </span>
        <span style="font-size: 0.85rem; color: #555; margin-left: 8px;">
            — Los archivos grandes (>30 MB) se procesan automáticamente vía Google Cloud Storage.
        </span>
    </div>
    """, unsafe_allow_html=True)

# --- Siempre usar st.file_uploader (funciona en todos los entornos) ---
_gcs_mode = None  # Ya no hay modo GCS separado; GCS se usa transparentemente en backend
...
if ready:
    ...
    with col2:
        if st.button("⚙️ Procesar y Analizar Programaciones", use_container_width=True, type="primary"):
            ...
            # ═══ GCS BACKEND: para archivos grandes, subir a GCS → descargar para procesar ═══
            if _GCS_AVAILABLE:
                _GCS_THRESHOLD_MB = 30  # Archivos > 30 MB pasan por GCS
                def _gcs_handle_large_file(uploaded_file):
                    """Si el archivo es grande y GCS está disponible, lo sube a GCS y lo devuelve como BytesIO."""
                    if uploaded_file is None:
                        return uploaded_file
                    try:
                        file_data = uploaded_file.getvalue()
                        size_mb = len(file_data) / (1024 * 1024)
                        if size_mb > _GCS_THRESHOLD_MB:
                            _update_progress(10, "☁️", f"Subiendo archivo grande ({size_mb:.0f} MB) a Cloud Storage")
                            from gcs_upload import generate_upload_url, download_blob, delete_blob
                            from google.cloud import storage as _gcs_storage
                            _gc = _gcs_storage.Client()
                            _bucket = _gc.bucket(GCS_BUCKET)
                            import uuid as _uuid
                            blob_name = f"uploads/{_uuid.uuid4().hex[:12]}/{uploaded_file.name}"
                            _blob = _bucket.blob(blob_name)
                            _blob.upload_from_string(file_data, content_type="application/pdf")
                            st.session_state['_gcs_blobs_to_clean'].append(blob_name)
                            # Descargar como BytesIO fresco (libera la copia de Streamlit)
                            buf = download_blob(blob_name)
                            _update_progress(12, "☁️", "Archivo procesado vía Cloud Storage")
                            return buf
                    except Exception as e:
                        # Si falla GCS, usar el archivo directo
                        pass
                    return uploaded_file

                f_init = [_gcs_handle_large_file(f) for f in f_init]
                f_final = [_gcs_handle_large_file(f) for f in f_final]
