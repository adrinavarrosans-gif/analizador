...
# === GCS Upload para archivos grandes (bypass Cloud Run 32 MB limit) ===
try:
    from gcs_upload import (
        GCS_ENABLED, GCS_BUCKET,
        generate_upload_url, download_blob, blob_exists,
        delete_blob, cleanup_blobs, get_blob_size,
    )
    from gcs_uploader_component import render_gcs_uploader
    _GCS_AVAILABLE = GCS_ENABLED
except ImportError:
...
# ═══════════════════════════════════════════════════════════════════════════
# CARGA DE ARCHIVOS — dual mode: GCS (archivos grandes) / Streamlit (normal)
# ═══════════════════════════════════════════════════════════════════════════

# --- Helper: inicializar signed URLs en session_state (una sola vez por sesión) ---
def _ensure_gcs_signed_urls():
    """Genera signed URLs para cada slot de upload si GCS está habilitado."""
    if not _GCS_AVAILABLE:
        return
    for slot in ('gcs_ini_a', 'gcs_ini_b', 'gcs_fin_a', 'gcs_fin_b',
                 'gcs_inicial', 'gcs_final'):
        url_key = f"{slot}_url"
        blob_key = f"{slot}_blob"
        if url_key not in st.session_state:
            try:
                url, blob_name = generate_upload_url(f"{slot}.pdf")
                st.session_state[url_key] = url
                st.session_state[blob_key] = blob_name
            except Exception as e:
                st.session_state[url_key] = None
                st.session_state[blob_key] = None

def _gcs_download_if_ready(blob_key: str):
    """Si el blob existe en GCS, lo descarga y devuelve BytesIO. Si no, None."""
    blob_name = st.session_state.get(blob_key)
    if not blob_name:
        return None
    try:
        if blob_exists(blob_name) and get_blob_size(blob_name) > 0:
            return download_blob(blob_name)
    except Exception:
        pass
    return None

# Track GCS blobs for cleanup after processing
if '_gcs_blobs_to_clean' not in st.session_state:
    st.session_state['_gcs_blobs_to_clean'] = []

if _GCS_AVAILABLE:
    _ensure_gcs_signed_urls()

# --- Modo GCS: upload directo al bucket (sin límite 32 MB) ---
if _GCS_AVAILABLE:
    st.markdown("""
    <div style="background: linear-gradient(135deg, #e8f8f5, #d1f2eb); border-radius: 10px; 
         padding: 12px 18px; margin-bottom: 16px; border-left: 4px solid #0D5F5D;">
        <span style="font-size: 1.1rem; font-weight: 600; color: #0D5F5D;">
            ☁️ Modo Cloud Storage activo
        </span>
        <span style="font-size: 0.85rem; color: #555; margin-left: 8px;">
            — Los archivos se suben directamente a Google Cloud Storage (sin límite de tamaño).
            Sube los archivos y luego pulsa <b>Procesar</b>.
        </span>
    </div>
    """, unsafe_allow_html=True)
    ...
    if base_sel == 'BCN':
        ...
        with c1:
            ...
            """, unsafe_allow_html=True)
            url_a = st.session_state.get('gcs_ini_a_url')
            blob_a = st.session_state.get('gcs_ini_a_blob', '')
            if url_a:
                render_gcs_uploader(url_a, blob_a, "Parte A (PDF)", key="gcs-ini-a")
            url_b = st.session_state.get('gcs_ini_b_url')
            blob_b = st.session_state.get('gcs_ini_b_blob', '')
            if url_b:
                render_gcs_uploader(url_b, blob_b, "Parte B (PDF)", key="gcs-ini-b")
            st.markdown("</div>", unsafe_allow_html=True)
        with c2:
            st.markdown("""<div class="upload-container">
                <div class="upload-header">📁 Programación FINAL</div>
            """, unsafe_allow_html=True)
            url_fa = st.session_state.get('gcs_fin_a_url')
            blob_fa = st.session_state.get('gcs_fin_a_blob', '')
            if url_fa:
                render_gcs_uploader(url_fa, blob_fa, "Parte A (PDF)", key="gcs-fin-a")
            url_fb = st.session_state.get('gcs_fin_b_url')
            blob_fb = st.session_state.get('gcs_fin_b_blob', '')
            if url_fb:
                render_gcs_uploader(url_fb, blob_fb, "Parte B (PDF)", key="gcs-fin-b")
            st.markdown("</div>", unsafe_allow_html=True)

        # En modo GCS, los archivos se descargan al pulsar Procesar
        ready = True  # El check real se hace al pulsar Procesar
        f_init = []  # Se rellenarán tras descargar de GCS
        f_final = []
        _gcs_mode = 'bcn'
    else:
        c1, c2 = st.columns(2)
        with c1:
            ...
            """, unsafe_allow_html=True)
            url_ini = st.session_state.get('gcs_inicial_url')
            blob_ini = st.session_state.get('gcs_inicial_blob', '')
            if url_ini:
                render_gcs_uploader(url_ini, blob_ini, "Selecciona PDF inicial", key="gcs-inicial")
            st.markdown("</div>", unsafe_allow_html=True)
        with c2:
            st.markdown("""<div class="upload-container">
                <div class="upload-header"><span class="upload-header-icon">📁</span> Programación FINAL</div>
            """, unsafe_allow_html=True)
            url_fin = st.session_state.get('gcs_final_url')
            blob_fin = st.session_state.get('gcs_final_blob', '')
            if url_fin:
                render_gcs_uploader(url_fin, blob_fin, "Selecciona PDF final", key="gcs-final")
            st.markdown("</div>", unsafe_allow_html=True)
...
# --- Modo estándar: st.file_uploader (archivos < 32 MB) ---
else:
    _gcs_mode = None
    if base_sel == 'BCN':
        st.markdown("#### 📂 Carga de Archivos (BCN requiere múltiples PDFs)")
...
if ready:
    ...
    with col2:
        if st.button("⚙️ Procesar y Analizar Programaciones", use_container_width=True, type="primary"):
            ...
            # ═══ GCS MODE: descargar archivos de Cloud Storage ═══
            if _GCS_AVAILABLE and _gcs_mode:
                _update_progress(10, "☁️", "Descargando archivos de Cloud Storage")
                _gcs_download_errors = []
                if _gcs_mode == 'bcn':
                    for bk in ('gcs_ini_a_blob', 'gcs_ini_b_blob'):
                        bio = _gcs_download_if_ready(bk)
                        if bio:
                            f_init.append(bio)
                            st.session_state['_gcs_blobs_to_clean'].append(st.session_state.get(bk))
                    for bk in ('gcs_fin_a_blob', 'gcs_fin_b_blob'):
                        bio = _gcs_download_if_ready(bk)
                        if bio:
                            f_final.append(bio)
                            st.session_state['_gcs_blobs_to_clean'].append(st.session_state.get(bk))
                else:  # single
                    bio_ini = _gcs_download_if_ready('gcs_inicial_blob')
                    if bio_ini:
                        f_init.append(bio_ini)
                        st.session_state['_gcs_blobs_to_clean'].append(st.session_state.get('gcs_inicial_blob'))
                    bio_fin = _gcs_download_if_ready('gcs_final_blob')
                    if bio_fin:
                        f_final.append(bio_fin)
                        st.session_state['_gcs_blobs_to_clean'].append(st.session_state.get('gcs_final_blob'))

                if not any(f for f in f_init if f) or not any(f for f in f_final if f):
                    progress_bar.empty()
                    status_text.empty()
                    pct_text.empty()
                    st.error("❌ **No se encontraron archivos en Cloud Storage.**")
                    st.warning("Asegúrate de que los archivos se han subido correctamente "
                               "(verifica el ✅ en cada slot) antes de pulsar Procesar.")
                    st.stop()

                _update_progress(12, "☁️", "Archivos descargados de Cloud Storage")
            ...
            if r_f:
                ...
                # ═══ GCS CLEANUP: borrar blobs temporales ═══
                if _GCS_AVAILABLE and st.session_state.get('_gcs_blobs_to_clean'):
                    try:
                        cleanup_blobs(st.session_state['_gcs_blobs_to_clean'])
                    except Exception:
                        pass
                    st.session_state['_gcs_blobs_to_clean'] = []
                    # Limpiar signed URLs para regenerar en próximo análisis
                    for slot in ('gcs_ini_a', 'gcs_ini_b', 'gcs_fin_a', 'gcs_fin_b',
                                 'gcs_inicial', 'gcs_final'):
                        st.session_state.pop(f"{slot}_url", None)
                        st.session_state.pop(f"{slot}_blob", None)
